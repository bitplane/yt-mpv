...
def notify(message: str) -> None:
    ...
    try:
        subprocess.run(["notify-send", "YouTube ULP", message], 
...
def check_dependencies() -> bool:
    ...
    if not os.path.isfile(os.path.join(VENV_BIN, "activate")):
        ...
        notify(f"Python venv missing: run install.sh")
        return False
...
def download_video(url: str) -> Optional[Tuple[Path, Path]]:
    ...
    status, stdout, stderr = run_command(
        [
            ...
            "--write-info-json",
            "-o", f"{DL_DIR}/yt-ulp-%(extractor)s-%(id)s.%(ext)s",
            url
        ],
    ...
    if not video_file.exists():
        video_file = next((f for f in video_file.parent.glob(f"{video_file.name}.*") 
                           if f.suffix != ".json"), None)
        if not video_file:
